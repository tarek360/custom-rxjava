...
import custom.reactivex.Completable;
...
import custom.reactivex.processors.FlowableProcessor;
import custom.reactivex.processors.UnicastProcessor;
 ...
 * <p>
 * When the {@link custom.reactivex.Scheduler.Worker Worker} is unsubscribed the {@link Completable} emits an
 ...
 * the number of concurrent {@link custom.reactivex.Scheduler.Worker Worker}s rather than individual actions.
 * Generally each {@link Observable} uses its own {@link custom.reactivex.Scheduler.Worker Worker}. This means
 * that this will essentially limit the number of concurrent subscribes. The
 * danger comes from using operators like
 * {@link Flowable#zip(org.reactivestreams.Publisher, org.reactivestreams.Publisher, custom.reactivex.functions.BiFunction)} where
